...
{
    public CreateUserCommandValidator(IFirstUnitOfWork unitOfWork)
    {
        RuleFor(x => x.Name)
        ...
        RuleFor(x => x.Email)
            ...
            {
                var user = await unitOfWork.UserRepository.GetUserByEmailAsync(currentEmail, cancellationToken);

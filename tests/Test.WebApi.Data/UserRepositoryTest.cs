...
public class UserRepositoryTest(SharedContext sharedContext) : RootTest(sharedContext)
{
    ...
    public async Task CreateUserAsync_Should_Return_NewUser()
    {
        ...
        // Act
        await UnitOfWork.UserRepository.CreateAsync(createUser);
    ...
    public async Task UpdateUserAsync_Should_Return_User_Updated()
    {
        ...
        // Act
        await UnitOfWork.UserRepository.UpdateAsync(updateUser);
    ...
    public async Task UpdateUserStatusAsync_Should_Return_Another_Status()
    {
        ...
        // Act
        await UnitOfWork.UserRepository.UpdateUserStatusAsync(updateStatusUser);
        // Assert
        var user = await UnitOfWork.UserRepository.GetByIdAsync(userId);
        Assert.NotNull(user);
    ...
    public async Task GetPageAsync_Should_Return_Page_1()
    {
        ...
        // Act
        var userPage = await UnitOfWork.UserRepository.GetPageAsync(pageNumber: pageNumber, pageSize: pageSize);
    ...
    public async Task GetPageAsync_Should_Return_Filter_By_Name()
    {
        ...
        // Act
        var userPage = await UnitOfWork.UserRepository.GetPageAsync(pageNumber: pageNumber, pageSize: pageSize, filterName: filterName);

        // Assert
        Assert.NotNull(userPage);
        Assert.All(userPage.List, (UserDto user) => user.Name.Equals(filterName, StringComparison.CurrentCultureIgnoreCase));
        Assert.All(userPage.List, (UserDto user) => user.FullName.Equals(filterName, StringComparison.CurrentCultureIgnoreCase));
    ...
    public async Task FilterAsync_Should_Return_Filter_By_Name()
    {
        ...
        // Act
        var userPage = await UnitOfWork.UserRepository.FilterAsync(filter);

        // Assert
        Assert.NotNull(userPage);
        Assert.All(userPage.List, (UserDto user) => user.Name.Equals(filter, StringComparison.CurrentCultureIgnoreCase));
        Assert.All(userPage.List, (UserDto user) => user.FullName.Equals(filter, StringComparison.CurrentCultureIgnoreCase));
